<template>
  <div class="backlog">
    <h5><i class="fas fa-cogs"></i>  Back-Log</h5>
      <div class="list">
        <ul>
          <li v-for="task of tasks" v-if="task.status == ['backlog']" v-bind:key="task['.key']">
            <h5>{{ task.title }}</h5>
            <p>Score: {{ task.point }}</p>
            <p>Assign to: {{ task.assign }}</p>
            
            <!-- Button trigger modal -->
            <button type="button" class="btn btn-info btn-sm btnctsm" data-toggle="modal" data-target="#modalBacklog" v-on:click="popModal(task)">
              Show Details
            </button>

            <!-- Modal -->
            <div class="modal fade" id="modalBacklog" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
              <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                  ...
                  <div class="modal-footer">
                    <!-- <button v-on:click="back()" class="left">Apalah</button> -->
                    <button v-on:click="deleteTask()" class="center btn btn-danger" data-dismiss="modal">Delete</button>
                    <button v-on:click="goToTodo()" class="right btn btn-primary" data-dismiss="modal">Todo</button>
                  </div>
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    goToTodo () {
      tasksRef.child(this.modalKey).update({ status: 'todo' })
    }
