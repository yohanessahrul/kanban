<template>
  <div class="done">
    <h5><i class="fas fa-clipboard-check"></i>  Done</h5>
      <div class="list">
        <ul>
          <li v-for="task of tasks" v-if="task.status == ['done']" v-bind:key="task['.key']">
            <h5>{{ task.title }}</h5>
            <p>Score: {{ task.point }}</p>
            <p>Assign to: {{ task.assign }}</p>
            
            <!-- Button trigger modal -->
            <button type="button" class="btn btn-info btn-sm btnctsm" data-toggle="modal" data-target="#modalDone" v-on:click="popModal(task)">
              Show Details
            </button>
              ...
              <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                  ...
                  <div class="modal-footer">
                    <button v-on:click="goToDoing()" class="left btn btn-warning" data-dismiss="modal">Doing</button>
                    <button v-on:click="deleteTask()" class="center btn btn-danger" data-dismiss="modal">Delete</button>
                    <!-- <button v-on:click="goToDoing()" class="right" data-dismiss="modal">Todo</button> -->
                  </div>
                </div>
              </div>
            </div>
